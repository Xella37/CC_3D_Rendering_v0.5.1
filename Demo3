...
local objects = { -- all objects in the game
	{model = "box", x = 4, y = 0.5, z = 0},
	{model = "spike", x = 4, y = 0.5, z = 2},
	{model = "spike", x = 4, y = 0.5, z = 4, rotationY = 45},
	{model = "emerald", x = 4, y = 0.5, z = 6},
	{model = "pineapple", x = 4, y = 0.5, z = 8, rotationY = 0},
}
	...
	while true do
		...
		elseif (event == "mouse_click") then
			...
			if (objectIndex ~= nil) then -- if the player clicked on an object (not void)
				local modelName = objects[objectIndex].model
				local model = ThreeDFrame:getModel(modelName)
				table.remove(model, polyIndex)
				ThreeDFrame:loadModel(modelName, model)
			end
		end
	end
end
